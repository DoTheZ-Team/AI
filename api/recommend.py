...
# 추천 결과 반환 API
@recommends_router.post("", response_model=Recommendations, summary="블로그 추천", description="블로그 id값과 팔로우된 블로그들의 정보를 바탕으로 블로그 추천을 해주는 API입니다. \n 블로그 서비스와 통신을 진행할 API입니다.")
async def recommend_blog(request: RecommendationRequest = Body(..., example={
    ...
    return Recommendations(recommendBlogId=recommend_blog_ids)